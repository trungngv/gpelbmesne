function [varargout] = mygp(post, hyp, inf, mean, cov, lik, x, y, xs, ys)
% Adopted version of the original GP to allow input of the covariance
% matrix to save computation when we need to make prediction for different
% set of test inputs.
%
...
%
%   training: [nlZ dnlZ          ] = gp(post, hyp, inf, mean, cov, lik, x, y);
% prediction: [ymu ys2 fmu fs2   ] = gp(post, hyp, inf, mean, cov, lik, x, y, xs);
%         or: [ymu ys2 fmu fs2 lp] = gp(post, hyp, inf, mean, cov, lik, x, y, xs, ys);
%
% where:
%
%   post     saved struct representation if available (must be [] if not
%            available)
...
if nargin<8 || nargin>10
  disp('Usage: [nlZ dnlZ          ] = mygp(post, hyp, inf, mean, cov, lik, x, y);')
  disp('   or: [ymu ys2 fmu fs2   ] = mygp(post, hyp, inf, mean, cov, lik, x, y, xs);')
  disp('   or: [ymu ys2 fmu fs2 lp] = mygp(post, hyp, inf, mean, cov, lik, x, y, xs, ys);')
  return
end
...
try                                                  % call the inference method
  ...
  if nargin>8 && isempty(post)   % compute marginal likelihood and its derivatives only if needed
  ...
  msgstr = lasterr;
  if nargin > 8, error('Inference method failed [%s]', msgstr); else 
...
if nargin==8                                     % if no test cases are provided
